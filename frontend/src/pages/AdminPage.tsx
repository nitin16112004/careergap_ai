...
const RolesSection = ({ roles, skills, busy, run }: { roles: AdminJobRole[]; skills: AdminSkill[]; busy: string; run: (key: string, action: () => Promise<unknown>, message: string) => Promise<void> }): JSX.Element => {
  ...
  const chooseEditRole = (id: string) => { const role = roles.find((item) => item.id === id); setEditRoleId(id); setEditRoleName(role?.role_name ?? ""); setEditCategory(role?.category ?? ""); };
  return <>
    <div className="admin-form-grid">
      ...
      <form className="admin-panel admin-form" onSubmit={submitEdit}><h2>Edit job role</h2><label className="admin-field"><span>Job role</span><select required value={editRoleId} onChange={(e) => chooseEditRole(e.target.value)}><option value="">Select role</option>{roles.map((role) => <option value={role.id} key={role.id}>{role.role_name}</option>)}</select></label><label className="admin-field"><span>Role name</span><input required value={editRoleName} onChange={(e) => setEditRoleName(e.target.value)} /></label><label className="admin-field"><span>Category</span><input value={editCategory} onChange={(e) => setEditCategory(e.target.value)} /></label><button className="admin-button" disabled={busy === "edit-role"}>Save role</button></form>
