...
6. `feat/billing-upgrade` — documented Phase 14 payment/upgrade flow built directly on Version 1.4, including usage enforcement, paid entitlements, Razorpay/Stripe provider boundaries, billing UI, webhook idempotency, and subscription/payment persistence.
...
- A Supabase project for real database/auth/storage/vector/reminder/billing integration.
...
See `.env.example`, `RAG_ROADMAP_SETUP.md`, `V1_3_PRODUCTION_HARDENING.md`, `V1_4_REMINDERS_SETUP.md`, and `BILLING_PAYMENT_SETUP.md`.
...
The repository is now at the documented **Phase 14 payment/upgrade implementation checkpoint**. Continue in this sequence:
...
7. Complete the documented admin application beyond the current indexing/operations/reminder APIs.
